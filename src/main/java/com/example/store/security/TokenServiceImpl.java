...
public class TokenServiceImpl {
    ...
    public String generateToken(Authentication authentication) {
        ...
        String token =  Jwts.builder()
                ...
                .setExpiration(convertToLocalDateToDate(LocalDateTime.now().plusSeconds(tokenProperties.getExpirationInSeconds())))
                .signWith(key(), SignatureAlgorithm.HS256)
                .compact();
        redisTemplate.opsForValue().set(userPrincipal.getUsername(), token, Duration.ofSeconds(tokenProperties.getExpirationInSeconds()));
        return token;
    }
